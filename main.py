...
async def vote_serveur_prive(username, proxy_conf=None):
    ...
    async with async_playwright() as p:
        ...
        # Désactivation du route interceptor pour le mode headless sur fly.io
        ...
        try:
            ...
            # Attendre plus longtemps en mode headless
            wait_time = 10000 if proxy_conf else 10000
            await page.wait_for_timeout(wait_time)  # Attendre que la page soit complètement chargée
            
            # Screenshot pour debug
            await page.screenshot(path=f"debug_serveur_prive_{username}.png")
            logging.info(f"Screenshot sauvegardé: debug_serveur_prive_{username}.png")
            ...
            try:
                vote_now_btn = await page.wait_for_selector(vote_btn_selectors, timeout=20000)
                if vote_now_btn:
                    ...
                    await page.wait_for_load_state('networkidle')
                    # Attendre plus longtemps en mode headless
                    vote_page_wait = 5000 if proxy_conf else 5000
                    await page.wait_for_timeout(vote_page_wait)
            except Exception as e:
                logging.warning(f"Bouton de vote initial non trouvé: {e}. On continue...")
                logging.info(f"URL actuelle: {page.url}")
                # Log pour debug: vérifier si on est déjà sur la page de vote
                if "vote" in page.url.lower():
                    logging.info("On est déjà sur une page de vote, on continue...")
            ...
            try:
                # Timeout augmenté pour le mode headless
                iframe_timeout = 30000 if proxy_conf else 30000
                ...
                await frame.wait_for_selector(".mtcap-image-mini", timeout=15000)
            except Exception as e:
                ...
                if "captcha" in page_content.lower():
                    logging.info("Le mot 'captcha' est présent dans la page mais l'iframe n'est pas trouvée")
                raise
...
async def vote_serveur_minecraft_vote(username, proxy_conf=None):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Screenshot pour debug
            await page.screenshot(path=f"debug_serveur_minecraft_vote_{username}.png")
            logging.info(f"Screenshot sauvegardé: debug_serveur_minecraft_vote_{username}.png")

            logging.info("🖱️ Recherche de l'onglet 'Voter'...")
            try:
                cookie_btn = await page.wait_for_selector(".js-cookie-consent-agree", timeout=5000)
                if cookie_btn and await cookie_btn.is_visible():
                    await cookie_btn.click()
            except:
                pass
            
            voter_tab = await page.wait_for_selector('a[href$="/vote"]', timeout=20000)
            if voter_tab:
                await voter_tab.click()
                await page.wait_for_load_state('networkidle')
                logging.info("✨ Onglet 'Voter' cliqué, page de vote chargée.")
            else:
                logging.error("❌ Onglet 'Voter' non trouvé. URL actuelle: {}".format(page.url))
                # Log du contenu pour debug
                content = await page.content()
                if "vote" in content.lower():
                    logging.info("Le mot 'vote' est dans la page mais le lien n'est pas trouvé")
                raise Exception("Onglet 'Voter' non trouvé")

            logging.info(f"✍️ Saisie du pseudo : {username}")
            pseudo_input = await page.wait_for_selector("input#pseudo", timeout=20000)
            if pseudo_input:
                await pseudo_input.click(force=True)
                await pseudo_input.fill(username)
                
            logging.info("🖱️ Recherche du bouton de vote...")
            vote_btn = await page.wait_for_selector("button#vote-button-action", timeout=20000)
...
async def vote_serveur_minecraft_com(username, proxy_conf=None):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Screenshot pour debug
            await page.screenshot(path=f"debug_serveur_minecraft_com_{username}.png")
            logging.info(f"Screenshot sauvegardé: debug_serveur_minecraft_com_{username}.png")
