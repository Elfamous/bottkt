...
async def vote_serveur_prive(username, proxy_conf=None):
    ...
    async with async_playwright() as p:
        ...
        # --- ROUTE INTERCEPTEUR MODÉRÉ (MODE HEADLESS) ---
        async def route_interceptor(route):
            res_type = route.request.resource_type
            url = route.request.url.lower()
            
            # Laisser passer tout ce qui est nécessaire pour le vote
            if "captcha" in url or "mtcaptcha" in url or "google" in url or "gstatic" in url or "serveur-prive" in url or "api" in url:
            ...
            # Bloquer seulement les ressources lourdes non essentielles
...
async def vote_serveur_minecraft_vote(username, proxy_conf=None):
    ...
    async with async_playwright() as p:
        ...
        async def route_interceptor(route):
            ...
            if "captcha" in url or "google" in url or "gstatic" in url or "api" in url:
                await route.continue_()
                return
            if res_type in ["xhr", "fetch", "websocket"]:
                await route.continue_()
